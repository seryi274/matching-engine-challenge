...
namespace exchange {
    ...
    OrderAck MatchingEngine::addOrder(const OrderRequest& request) {
        ...
        if (request.side == Side::Buy) {
            ...
            // asks is sorted ascending. we want to look for the first order such
            // that sell.price > buy_price
            auto first_greater_it = book.asks.upper_bound(buy_price);


        } else {
            auto& sell_price = request.price;
            // bids is sorted descending. we want to look for the first order such
            // tht buy.price < sell.price
            auto first_less_it = book.bids.upper_bound()
        }
