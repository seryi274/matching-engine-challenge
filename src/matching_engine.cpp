...
OrderStatus MatchingEngine::matchOrder(const Order& request) {
    ...
    std::set<uint64_t, OrderCmp>& orders_to_match = book[(1^request.bookIndex)][request.symbol];

    while (remaining_quantity > 0 && !orders_to_match.empty() && 
           orders_to_match.begin()->price * request.multiplier <= request.price * request.multiplier) {
        // We have a trade.
        const Order& o = existingOrders[*orders_to_match.begin()];
        const int64_t current_price = orders_to_match.begin()->price;
        const uint32_t volume = std::min(remaining_quantity, o.quantity);
        uint64_t buy_side = request.order_id;
        uint64_t sell_side = o.order_id;
        if (request.bookIndex == 1) std::swap(buy_side, sell_side);
        listener_->onTrade(Trade{buy_side, sell_side, symbols[request.symbol], current_price, volume});

        if (volume >= orders_to_match.begin()->quantity) {
            listener_->onOrderUpdate(OrderUpdate{o.order_id, OrderStatus::Filled, 0});
            existingOrders.erase(o.order_id);
            orders_to_match.erase(orders_to_match.begin());
        } else {
            Order toModify = existingOrders[o.order_id];
            toModify.quantity -= volume;
            existingOrders[o.order_id] = toModify;
            orders_to_match.erase(orders_to_match.begin());
            orders_to_match.insert(toModify.order_id);
        }
    ...
    if (remaining_quantity == 0) {
        ...
        book[request.bookIndex][request.symbol].insert(request);
    }
...
OrderAck MatchingEngine::addOrder(const OrderRequest& request) {
   ...
   const uint8_t bookSide = (request.side == Side::Buy ? 0 : 1);
    ...
    if (stockToId.find(request.symbol) == stockToId.end()) {
        ...
        std::set<uint64_t, OrderCmp> toAdd;
    ...
    new_order.symbol = stockToId[request.symbol];
    new_order.bookIndex = (request.side == Side::Buy ? 0 : 1);
    new_order.multiplier = (request.side == Side::Buy ? 1 : -1);
...
bool MatchingEngine::cancelOrder(uint64_t order_id) {
    ...
    book[toErase.bookIndex][toErase.symbol].erase(toErase.order_id);
...
bool MatchingEngine::amendOrder(uint64_t order_id, int64_t new_price, uint32_t new_quantity) {
   ...
   book[toModify.bookIndex][toModify.symbol].erase(toModify.order_id);
   if (new_price != toModify.price || toModify.quantity < new_quantity) {
        ...
        book[toModify.bookIndex][toModify.symbol].insert(toModify.order_id);
        matchOrder(toModify);
    } else {
        toModify.quantity = new_quantity;
        book[toModify.bookIndex][toModify.symbol].insert(toModify.order_id);
   ...
   const std::string& symbol, Side side) const
{
    ...
    last->price = existingOrders[*book[bookSide][id].cbegin()].price;
    last->total_quantity = 0;
    last->order_count = 0;
    // []
    // *it = 
    for (auto itt = book[bookSide][id].cbegin(); itt != book[bookSide][id].cend(); ++itt){
        const Order& it = existingOrders[*itt];
